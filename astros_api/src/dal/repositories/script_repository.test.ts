...
describe("I2cRepository", () => {
  ...
  it("should save script with script events", async () => {
    ...
    const i2cIdx = savedScripts[0].scriptChannels.findIndex(
      (c) => c.channelType === ScriptChannelType.GENERIC_I2C,
    );

    expect(i2cIdx).toBeGreaterThan(-1);
    expect(savedScripts[0].scriptChannels[i2cIdx].channelType).toBe(
      ScriptChannelType.GENERIC_I2C,
    );

    const gpioIdx = savedScripts[0].scriptChannels.findIndex(
      (c) => c.channelType === ScriptChannelType.GPIO,
    );

    expect(gpioIdx).toBeGreaterThan(-1);
    expect(savedScripts[0].scriptChannels[gpioIdx].channelType).toBe(
      ScriptChannelType.GPIO,
    );

    const uartIdx = savedScripts[0].scriptChannels.findIndex(
      (c) => c.channelType === ScriptChannelType.GENERIC_UART,
    );

    expect(uartIdx).toBeGreaterThan(-1);
    expect(savedScripts[0].scriptChannels[uartIdx].channelType).toBe(
      ScriptChannelType.GENERIC_UART,
    );

    const kangarooIdx = savedScripts[0].scriptChannels.findIndex(
      (c) => c.channelType === ScriptChannelType.KANGAROO,
    );

    expect(kangarooIdx).toBeGreaterThan(-1);
    expect(savedScripts[0].scriptChannels[kangarooIdx].channelType).toBe(
      ScriptChannelType.KANGAROO,
    );

    const maestroServoIdx = savedScripts[0].scriptChannels.findIndex(
      (c) =>
        c.channelType === ScriptChannelType.SERVO &&
        c.moduleChannelType === ModuleChannelTypes.MaestroChannel,
    );

    expect(maestroServoIdx).toBeGreaterThan(-1);
    expect(savedScripts[0].scriptChannels[maestroServoIdx].channelType).toBe(
      ScriptChannelType.SERVO,
    );

    const maestroGpioIdx = savedScripts[0].scriptChannels.findIndex(
      (c) =>
        c.channelType === ScriptChannelType.GPIO &&
        c.moduleChannelType === ModuleChannelTypes.MaestroChannel,
    );

    expect(maestroGpioIdx).toBeGreaterThan(-1);
    expect(savedScripts[0].scriptChannels[maestroGpioIdx].channelType).toBe(
      ScriptChannelType.GPIO,
    );
  });
});
...
async function populateDatabase(
  ...
  maestroSubModule.boards.push(
    new MaestroBoard(uuid(), maestroModule.id, 0, "Board 0", 24),
  );

  for (let i = 0; i < 24; i++) {
    maestroSubModule.boards[0].channels.push(
      new MaestroChannel(
        uuid(),
        maestroModule.id,
        `Channel ${i}`,
...
function createI2cChannel(scriptId: string, location: ControllerLocation) {
  ...
  const channel = new I2cChannel(uuid(), i2cModule.id, i2cModule.name, true);
...
function createUartChannel(scriptId: string, location: ControllerLocation) {
  const idx = location.uartModules.findIndex(
    (m) => m.moduleSubType === ModuleSubType.genericSerial,
  );
...
function createKangarooChannel(scriptId: string, location: ControllerLocation) {
  const idx = location.uartModules.findIndex(
    (m) => m.moduleSubType === ModuleSubType.kangaroo,
  );
...
function createMaestroServoChannel(
  scriptId: string,
  location: ControllerLocation,
) {
  const idx = location.uartModules.findIndex(
    (m) => m.moduleSubType === ModuleSubType.maestro,
  );
...
function createMaestroGpioChannel(
  scriptId: string,
  location: ControllerLocation,
) {
  const idx = location.uartModules.findIndex(
