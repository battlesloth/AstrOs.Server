...
export function registerAudioRoutes(router: Router, auth: any) {
  router.get(getAll, auth, getAllAudioFiles);
  router.delete(deleteRoute, auth, deleteAudioFile);
}

export async function getAllAudioFiles(req: any, res: any, next: any) {
  try {
...
export async function deleteAudioFile(req: any, res: any, next: any) {
  try {
    const repo = new AudioFileRepository(db);
