...
async function validateGpioChannel(page: Page, location: string, ch: number) {
  await expect(page.getByTestId(`${location}-gpio-ch-${ch}-name`)).toHaveValue('unassigned');
  await expect(page.getByTestId(`${location}-gpio-ch-${ch}-enabled`)).not.toBeChecked();
  await expect(page.getByTestId(`${location}-gpio-ch-${ch}-default-high`)).not.toBeChecked();
}
