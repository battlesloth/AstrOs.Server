...
<template>
  <div class="space-y-4">
    <div class="flex flex-row gap-5">
      ...
      <select :data-testid="`${parentTestId}-kangaroo-baud`" v-model="baudRate" @change="onBaudRateChange(baudRate)"
    ...
    <div v-if="subModule" class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="flex items-center gap-2">
        ...
        <input :data-testid="`${parentTestId}-kangaroo-ch1Name`" v-model="subModule.ch1Name"
          :placeholder="$t('uart.channel_1')" class="input input-bordered input-sm grow" />
      </div>
      <div class="flex items-center gap-2">
        ...
        <input :data-testid="`${parentTestId}-kangaroo-ch2Name`" v-model="subModule.ch2Name"
          :placeholder="$t('uart.channel_2')" class="input input-bordered input-sm grow" />
      </div>
    </div>
  </div>
</template>
