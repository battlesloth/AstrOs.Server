...
<template>
  <div class="flex flex-wrap gap-4">
    ...
    <div class="flex items-center gap-4 min-w-87.5 grow">
      <div class="text-lg font-medium min-w-27.5">Channel {{ channel.channelNumber }}</div>
      <div class="grow">
        <input
          :data-testid="`${parentTestId}-gpio-ch-${channel.channelNumber}-name`"
    ...
    <div class="flex items-center gap-4 min-w-87.5 grow">
      <div class="form-control w-30">
        <label class="label cursor-pointer justify-center gap-2">
          <input
            type="checkbox"
            :data-testid="`${parentTestId}-gpio-ch-${channel.channelNumber}-enabled`"
      ...
      <div class="form-control w-37.5">
        <label class="label cursor-pointer justify-center gap-2">
          <input
            type="checkbox"
            :data-testid="`${parentTestId}-gpio-ch-${channel.channelNumber}-default-high`"
            v-model="channel.defaultHigh"
            class="checkbox checkbox-sm"
          />
          <span class="label-text">Default High?</span>
        </label>
      </div>
    </div>
  </div>
