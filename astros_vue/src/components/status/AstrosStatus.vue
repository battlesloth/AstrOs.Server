...
<template>
    <div class="flex justify-center">
        ...
        <img v-show="bodyStatus !== ControllerStatus.UP" :src="bodyImg" class="
            absolute z-10 max-w-72 animate-pulse" :alt="`Body status ${bodyStatus}`" />
        <img v-show="domeStatus !== ControllerStatus.UP" :src="domeImg" class="
            absolute z-10 max-w-72 animate-pulse" :alt="`Dome status ${domeStatus}`" />
        <img v-show="coreStatus !== ControllerStatus.UP" :src="coreImg" class="
            absolute z-10 max-w-72 animate-pulse" :alt="`Core status ${coreStatus}`" />
    </div>
</template>
