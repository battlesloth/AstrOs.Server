...
import type { LocationDetails, ChannelDetails, AddChannelModalResponse } from '@/models/scripts/scripting';
...
<template>
  <dialog class="modal modal-open">
    <div class="modal-box w-100 max-w-md">
      ...
      <div class="py-4">
        <select id="controller-select" v-model="selectedController" title="Controller"
          class="select select-bordered w-full text-2xl mb-5">
          <option v-for="c in controllersWithAny" :key="c.id" :value="c.id">
            {{ c.name }}
          </option>
        </select>

        <select id="module-select" v-model="selectedChannelType" title="Channel Type"
          class="select select-bordered w-full text-2xl mb-5">
          <option v-for="ch in channelTypes" :key="ch.id" :value="ch.id">
            {{ ch.name }}
          </option>
        </select>

        <select id="channel-select" v-model="selectedChannels" title="Channel" multiple
          class="select select-bordered w-full text-2xl mb-5 min-h-50">
          <option value="-1" disabled>Select Channel</option>
          <option v-for="ch in filteredChannels" :key="ch.id" :value="ch.id" :disabled="!ch.available">
            {{ ch.name }}
          </option>
        </select>
      ...
      <div class="modal-action justify-center">
        <button class="btn btn-primary w-24 text-lg" data-testid="add-channel-button" @click="addChannel">
          Add
        </button>
