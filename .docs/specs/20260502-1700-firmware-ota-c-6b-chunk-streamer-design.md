...
- **`base64` encoding of chunk payloads and CRC-16 computation are the streamer's responsibility, not the generator's.** `MessageGenerator.generateFwChunk()` (c.1, in `astros_api/src/serial/message_generator.ts`) only serializes the wire frame — it joins `transferId | seq | payloadLen | base64Bytes | crc16Hex` with the protocol's separator and wraps with the GS / EOL framing. It expects `base64Bytes` and `crc16Hex` to already be populated on the `FwChunk` payload. The streamer calls `chunkBytes.toString('base64')` for the encoding (already wired) and is also where the CRC-16/CCITT-FALSE helper must be invoked. **CRC TODO**: `chunk_streamer.ts` currently passes `crc16Hex: 'TODO_TASK_4_CRC16'` (a non-numeric greppable marker, NOT `'0000'` — a buggy validator can't silently accept it as a zero-byte CRC). The real helper lands when the master starts checking it; until then, `FakeSerialBus` doesn't validate, so the placeholder is wire-shape-correct enough for the state-machine tests.
