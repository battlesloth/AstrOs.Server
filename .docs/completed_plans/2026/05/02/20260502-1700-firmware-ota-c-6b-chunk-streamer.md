...
- [x] **`ChunkStreamer` skeleton + happy-path single-chunk transfer** (new `astros_api/src/firmware/chunk_streamer.ts`):
  ...
  - Uses `MessageGenerator` from `astros_api/src/serial/message_generator.ts` for outbound payload **framing** (separator joins + GS/EOL wrap). The streamer is responsible for the **bytes-level encoding**: base64 of the chunk slice (`chunkBytes.toString('base64')`) and CRC-16 of the chunk bytes (currently a `'TODO_TASK_4_CRC16'` placeholder pending the real CRC-16/CCITT-FALSE helper). `MessageGenerator.generateFwChunk` only serializes whatever `base64Bytes` / `crc16Hex` the caller hands it.
...
- **`MessageGenerator` is the framing source of truth, but not the bytes-level encoding source.** c.6b doesn't reimplement the line-delimited framing (separator joins + GS / EOL wrap) — that lives in `generateFwTransferBegin`, `generateFwChunk`, `generateFwTransferEnd` from c.1. The streamer DOES own base64 encoding (`chunkBytes.toString('base64')`) and CRC-16 computation — the generator just packages whatever `base64Bytes` / `crc16Hex` strings the caller provides on the `FwChunk` payload. The CRC helper is a `'TODO_TASK_4_CRC16'` placeholder until the master starts checking it.
