...
- [ ] **Banner tests + story.** Add `AstrosLockStateBanner.spec.ts` covering: hidden when not locked; visible when locked + not own-job; hidden when locked + own-job; hidden when readonly + locked (readonly precedence); CTA renders on non-firmware routes only. Add `AstrosLockStateBanner.stories.ts` with Visible / Hidden-OwnJob / Hidden-Readonly / Hidden-Unlocked stories driven by Pinia + router stubs (use the same stub pattern as `AstrosWriteButton.stories.ts`).
