...
- [ ] **Task 1 — Typed models + WS event enum** (new `astros_api/src/models/firmware/flash_orchestrator.ts`; modify `astros_api/src/models/enums.ts`):
  - In `models/enums.ts`, replace placeholder `flashJobActive` with the 5 new event types in `TransmissionType` enum (in this order so existing numeric values aren't shifted unexpectedly): `flashJobStarted, flashControllerUpdate, flashControllerResult, flashJobDone, flashJobFailed`. Keep `lockStateChanged` adjacent. Note: this changes numeric enum values; verify no consumer hardcodes them (grep + tests).
...
- **Why `flashJobActive` was removed rather than retained.** It was added pre-c.6 as a placeholder and is not currently consumed (verified by grep). Replacing with the 5 specific event types matches the spec's vocabulary 1:1.
