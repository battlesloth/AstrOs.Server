...
- [x] **`FirmwareCache` class** (new `astros_api/src/firmware/firmware_cache.ts`):
    ...
    - `fetch(release: ReleaseInfo, asset: AssetInfo): Promise<CachedAsset>` — if `lookup()` hits, return that. Otherwise download to `<name>.tmp` while streaming through `crypto.createHash('sha256')`; on success, write sidecars, atomic-rename `.tmp` → `.bin`, prune to N=5, return. Cleanup is symmetric across both failure phases: a download or hash error unlinks the `.tmp`; a persist-phase error (writeFile EACCES/ENOSPC, rename EACCES/EXDEV after sidecars have already landed) unlinks the `.tmp` *and* both sidecars so a retry has the same clean slate as a download failure — without this, `lookup()` correctly treats the partial state as a miss but the orphaned `.tmp` + sidecars accumulate every retry until something else evicts them. Takes both `release` and `asset` because the meta sidecar needs `tag` and `publishedAt` from the parent release (`AssetInfo` doesn't carry them); the orchestrator naturally has both in scope.
...
- [x] **Tests** (new `astros_api/src/firmware/firmware_cache.test.ts`). TDD with mocked fetcher + temp dir per test (`fs.mkdtempSync` in `beforeEach`, `rm -rf` in `afterEach`). Cover:
    ...
    - `fetch` removes the `.tmp` and both sidecars when the rename to `.bin` fails after sidecars are written (rename EACCES)
