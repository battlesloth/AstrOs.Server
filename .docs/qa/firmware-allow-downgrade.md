...
If a controller is reachable (status='up' or 'needsSynced') but its firmware version is unknown / unparseable, `compareTags` returns NaN and the downgrade gate fails open: the row is selectable, the toggle doesn't appear (no fleet downgrade detected), and the modal ack region doesn't render. The operator may flash without realizing they are downgrading. This is a long-standing latent issue (predates this feature) — surfaced during the pre-push review. The `?? '—'` sentinel inside the `controllers` computed projection in `stores/firmware.ts` is the canonical reproduction (line number subject to drift).

A naive follow-up — promoting unparseable-current to `isHardBlocked` — would itself introduce a new silent failure: the row would project via `selectModePillKind` as `offline` even when the controller is reachable, making the operator think the controller is unreachable when it's actually just running firmware we can't parse. The correct fix likely needs a distinct `unknownFirmware` pill kind (an addition to `FirmwareStatusPillKind`) so the row reads as "reachable but version-unknown" rather than reusing the offline visual. Worth scoping separately.
